...
main()
{
   ...
   bin="${prefix}/bin"
   libexec="${prefix}/libexec/mulle-project"
   share="${prefix}/share/mulle-project"
   ...
   do
      ...
      install -v -m "644" "${i}" "${libexec}" || exit 1
   done

   for i in *.template mulle-*.png
   do
      mkdir -p "${share}" 2> /dev/null
      install -v -m "644" "${i}" "${share}" || exit 1
   done
}
